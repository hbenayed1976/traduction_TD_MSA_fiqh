"""
Evaluation autonome — LLaMA 3-8B fine-tune (adapteur LoRA)  v4
Task  : Traduction dialecte tunisien → arabe standard moderne (MSA)
Charge le modele de base + l'adapteur LoRA sauvegarde, puis evalue
sur le test set complet (test_set_FINAL.json).

Adapte de ALLaM-7B -> LLaMA 3-8B (Meta-Llama-3-8B-Instruct)
Differences cles :
  - MODEL_ID : meta-llama/Meta-Llama-3-8B-Instruct
  - trust_remote_code supprime (non necessaire pour LLaMA 3)
  - Format prompt : balises LLaMA 3 chat (<|begin_of_text|>, etc.)
  - Fix dtype bfloat16 conserve

v4 — NOTE : Ce script est un evaluateur AUTONOME (pas de Trainer).
  Le BLEU corpus est calcule directement sur toutes les predictions
  (section 5) — methode correcte pour l'evaluation finale.
  Le compute_metrics BLEU a ete ajoute dans le script de fine-tuning
  (finetune_llama3_8b_tun_msa_v4000.py) pour guider l'optimisation
  PENDANT l'entrainement.
  Ici, pointez LORA_DIR vers le meilleur checkpoint issu du FT v4
  (early stopping, LoRA r=32, dropout=0.10, wd=0.10 — experience v4-r32).
...
# ══════════════════════════════════════════════════════════════
MODEL_ID     = "meta-llama/Meta-Llama-3-8B-Instruct"   # modèle de base HF
# Pointez vers le meilleur checkpoint v4-r32 (early stop, r=32)
LORA_DIR     = r"C:\Users\infocom\tradiction_dialect\llama3_8b_tun_msa_r32_output-3000"
...
print(SEP)
print("  Evaluation LLaMA 3-8B LoRA v4-r32 — Traduction Tunisien -> MSA")
print(SEP)
...
# LLaMA 3 ne nécessite pas trust_remote_code=True
# ══════════════════════════════════════════════════════════════
...
# ── FIX DTYPE ────────────────────────────────────────────────
# Certains modules (lm_head, embed_tokens) peuvent rester en float32
...
# ══════════════════════════════════════════════════════════════
# 3. TEMPLATE D'INFÉRENCE
# Utilise le format de chat natif LLaMA 3 avec les balises spéciales.
# Doit être identique au format utilisé pendant l'entraînement.
# ══════════════════════════════════════════════════════════════
INFERENCE_TEMPLATE = (
    "<|begin_of_text|>"
...
def translate_to_msa(item: dict, max_new_tokens: int = 128) -> str:
    ...
    # tokenisation + cast sur le device du modèle
...
# ── Appel export ──────────────────────────────────────────────
excel_path = os.path.join(OUTPUT_DIR, "llama3_r32_evaluation_expert.xlsx")
...
print("\n" + SEP)
print("  ✅ EVALUATION LLaMA 3-8B v4-r32 TERMINEE AVEC SUCCES!")
print(SEP)
