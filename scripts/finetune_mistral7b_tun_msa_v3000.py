"""
Fine-tuning Mistral-7B-Instruct sur machine GPU/CUDA (Linux/Windows)
...
v3 — Améliorations :
  ...
  • Adapté pour Mistral-7B-Instruct-v0.3 (format prompt [INST])
"""
...
print(SEP)
print("  Fine-tuning Mistral-7B-Instruct — Traduction Tunisien → MSA  v3")
print(SEP)
...
OUTPUT_DIR   = r"C:\Users\infocom\tradiction_dialect\mistral7b_tun_msa_output-3000"
# Mistral-7B-Instruct v0.3 (HuggingFace)
...
# ══════════════════════════════════════════════════════════════
# 3. FORMAT DU PROMPT (Mistral [INST] format)
# ══════════════════════════════════════════════════════════════
# Mistral-Instruct utilise le format de chat :
#   <s>[INST] {instruction}\n\n{input} [/INST] {output}</s>
PROMPT_TEMPLATE = (
    "<s>[INST] {instruction}\n\n{input} [/INST] {output}</s>"
)
...
model = AutoModelForCausalLM.from_pretrained(
    ...
    # Mistral ne nécessite pas trust_remote_code=True (modèle natif HF)
)
...
# Mistral n'a pas de pad_token par défaut → utiliser eos_token
...
peft_config = LoraConfig(
    ...
    lora_alpha=128,
    lora_dropout=0.05,
    # Modules cibles pour l'architecture Mistral (identiques à LLaMA)
    target_modules=[
...
def export_excel(results: list, metrics: dict, out_path: str):
    ...
    t = ws_sum["A1"]
    t.value     = "Résumé des métriques — Mistral-7B-Instruct Fine-tuning"
    ...
    expert_headers = [
        ...
        "Référence MSA", "Traduction Mistral",
        "Exact Match",
    ...
    ws_exp["M1"].comment = Comment(score_guide, "Mistral Evaluation")
    ...
    # ─────────────────────────────────────────────────────────
    # ONGLET 3 : Échantillons aléatoires
    # ─────────────────────────────────────────────────────────
    ...
    sample_headers = ["ID", "Texte Tunisien", "Référence MSA",
                      "Traduction Mistral", "Token F1", "BLEU",
                      "Score Expert (0–5)", "Commentaire Expert"]
    ...
    # ─────────────────────────────────────────────────────────
    # ONGLET 4 : Distribution des métriques
    # ─────────────────────────────────────────────────────────
    ...
    bleu_vals  = [r["bleu"] / 100 for r in results]
...
# ── Appel export ──────────────────────────────────────────────
excel_path = f"{OUTPUT_DIR}/mistral_evaluation_expert.xlsx"
...
print("\n" + SEP)
print("  ✅ FINE-TUNING ET ÉVALUATION Mistral-7B-Instruct TERMINÉS AVEC SUCCÈS!")
print(SEP)
