...
evaluate_arat5v2.py
--------------------
Évaluation du modèle AraT5v2 fine-tuné pour la traduction
Dialecte Tunisien → Arabe classique (MSA).

Compatible avec le pipeline finetune_arat5v2_tun_msa.py :
  - Modèle   : UBC-NLP/AraT5v2-base-1024
  - Dataset  : JSON au format {"instruction": ..., "input": ..., "output": ...}
  - Préfixe  : "ترجم من اللهجة التونسية إلى العربية الفصحى: " (obligatoire pour T5)
  - Métriques : BLEU, chrF++, TER, ROUGE-1/2/L, Token-F1, Exact Match
  - Export   : JSON + Excel (4 onglets)

Prérequis :
    pip install transformers torch sacrebleu rouge-score openpyxl tqdm sentencepiece
"""
...
RESULTS_EXCEL = os.path.join(OUTPUT_DIR, "arat5v2_evaluation_expert.xlsx")

MODEL_ID    = "UBC-NLP/AraT5v2-base-1024"   # identique au fine-tuning
# T5 nécessite un préfixe de tâche explicite (contrairement à BART)
...
def load_model(model_dir: str):
    """Charge le tokenizer et le modèle fine-tuné AraT5v2 depuis model_dir."""
...
def translate_to_msa(item: dict, tokenizer, model) -> str:
    """
    Traduction AraT5v2 avec beam search.
    T5 requiert le préfixe de tâche TASK_PREFIX en entrée.
    """
...
def export_excel(results: list[dict], summary: dict, out_path: str) -> None:
    ...
    c = ws["A1"]
    c.value = "AraT5v2 — Évaluation Traduction Tunisien → MSA"
    ...
    we.freeze_panes = "A2"
    headers = [
        "ID", "Texte Tunisien", "Référence MSA", "Traduction AraT5v2",
    ...
    for col, h in enumerate(
        ["ID", "Texte Tunisien", "Référence MSA",
         "Traduction AraT5v2", "Token F1", "BLEU", "Score Expert", "Commentaire"], 1
    ):
...
def main():
    print(SEP)
    print("  AraT5v2 — Évaluation Traduction Tunisien → MSA")
    print(SEP)
    print(f"  Device      : {DEVICE}")
    print(f"  Modèle dir  : {OUTPUT_DIR}")
    print(f"  Préfixe T5  : {TASK_PREFIX}\n")
    ...
    print("\n" + SEP)
    print("  ✅ ÉVALUATION AraT5v2 TERMINÉE")
    print(SEP)
