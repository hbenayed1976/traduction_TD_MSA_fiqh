"""
Fine-tuning AraT5v2 (seq2seq) — Traduction dialecte tunisien → MSA
Modèle : UBC-NLP/AraT5v2-base-1024
Méthode : Fine-tuning complet (ou LoRA optionnel) en seq2seq
...
print(SEP)
print("  Fine-tuning AraT5v2 — Traduction Tunisien → MSA")
print(SEP)
...
# ══════════════════════════════════════════════════════════════
# 3. TOKENIZER + MODÈLE
# AraT5v2 : modèle T5 pré-entraîné sur corpus arabe massif
# Longueur max 1024 tokens (version base-1024)
# ══════════════════════════════════════════════════════════════
MAX_SRC = 128    # longueur max source (dialecte tunisien)
MAX_TGT = 128    # longueur max cible (MSA)
...
# ══════════════════════════════════════════════════════════════
# 4. PRÉFIXE DE TÂCHE (style T5)
# Un préfixe explicite aide T5 à cibler la tâche
# ══════════════════════════════════════════════════════════════
TASK_PREFIX = "ترجم من اللهجة التونسية إلى العربية الفصحى: "
...
def preprocess(batch):
    ...
    """
    sources = [TASK_PREFIX + s.strip() for s in batch["input"]]
...
train_ds_raw = Dataset.from_list(train_data)
val_ds_raw   = Dataset.from_list(test_data[:80])
test_ds_raw  = Dataset.from_list(test_data)
...
data_collator = DataCollatorForSeq2Seq(
    tokenizer,
    model=model,
    label_pad_token_id=-100,   # ignore padding dans la loss
...
training_args = Seq2SeqTrainingArguments(
    ...
    per_device_train_batch_size=16,   # seq2seq plus léger en VRAM
    per_device_eval_batch_size=16,
    gradient_accumulation_steps=2,
    num_train_epochs=10,              # plus d'époques que les LLMs
    learning_rate=5e-4,
    ...
    bf16=True,
    fp16=False,
    predict_with_generate=True,       # génération lors de l'éval
...
print(SEP)
print("  🔥 DÉBUT ENTRAÎNEMENT AraT5v2")
print(SEP)
...
def translate_to_msa(item: dict, num_beams: int = 4) -> str:
    """Traduction seq2seq avec beam search."""
    src = TASK_PREFIX + item["input"].strip()
    inputs = tokenizer(
        src,
        return_tensors="pt",
        max_length=MAX_SRC,
...
print("\n" + SEP)
print("  🎯 RÉSUMÉ DES MÉTRIQUES — AraT5v2 (corpus)")
print(SEP)
...
def export_excel(results, summary, out_path):
    ...
    c = ws["A1"]
    c.value = "AraT5v2 LoRA — Évaluation Traduction Tunisien → MSA"
    ...
    we.freeze_panes = "A2"
    headers = ["ID","Texte Tunisien","Référence MSA","Traduction AraT5v2",
    ...
    for col, h in enumerate(["ID","Texte Tunisien","Référence MSA",
                              "Traduction AraT5v2","Token F1","BLEU",
...
excel_path = f"{OUTPUT_DIR}/arat5v2_evaluation_expert.xlsx"
export_excel(results_eval, summary_metrics, excel_path)

print(f"\n💾 JSON  → {results_path}")
print(f"📊 Excel → {excel_path}")
print("\n" + SEP)
print("  ✅ FINE-TUNING ET ÉVALUATION AraT5v2 TERMINÉS!")
print(SEP)
