"""
Fine-tuning ALLaM-7B sur machine GPU/CUDA (Linux/Windows)
...
v3-r16e3 — RE-RUN DE LA MEILLEURE CONFIG (r=16, alpha=32, 3 epochs) :
  • Config LoRA alignée sur la meilleure configuration retenue dans l'article
    (r=16, alpha=32 = 2x r, 3 epochs) au lieu de la config générique r=64/4-epochs
  • Dossier de sortie renommé pour cohérence avec le script d'évaluation
    beam search (evaluate_allam7b_beam_search.py → LORA_DIR "*_r16_e3")
...
print(SEP)
print("  Fine-tuning ALLaM-7B — Traduction Tunisien → MSA  v3")
print(SEP)
...
OUTPUT_DIR   = r"C:\Users\infocom\tradiction_dialect\allam7b_tun_msa_output-3000_r16_e3"
...
model = AutoModelForCausalLM.from_pretrained(
    ...
    torch_dtype=torch.bfloat16,  # ou dtype= si transformers>=4.45
)
...
training_args = TrainingArguments(
    ...
    num_train_epochs=3,
    learning_rate=3e-4,   # inchangé (identique à la config r=64/4-epochs)
...
def export_excel(results: list, metrics: dict, out_path: str):
    ...
    t = ws_sum["A1"]
    t.value     = "Résumé des métriques — ALLaM-7B Fine-tuning"
...
# ── Appel export ──────────────────────────────────────────────
excel_path = f"{OUTPUT_DIR}/allam_evaluation_expert.xlsx"
...
print("\n" + SEP)
print("  ✅ FINE-TUNING ET ÉVALUATION ALLaM-7B TERMINÉS AVEC SUCCÈS!")
print(SEP)
