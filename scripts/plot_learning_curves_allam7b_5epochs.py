"""
plot_learning_curves_allam7b.py
--------------------------------
Visualisation des courbes d'apprentissage du modèle ALLaM-7B.
Compatible avec finetune_allam7b_tun_msa_v4000.py.

Paramètres du fine-tuning (v4) :
  - num_train_epochs        : 3  (+ Early Stopping patience=1 sur eval_bleu)
...
OUTPUT_DIR = r"C:\Users\infocom\tradiction_dialect\allam7b_tun_msa_output-4000"
SAVE_PATH  = os.path.join(OUTPUT_DIR, "courbe_apprentissage_allam7b.png")
SEP        = "=" * 60
...
def find_best_step(state: dict) -> int | None:
    """
    Récupère le step du meilleur modèle sauvegardé par le Trainer.
    (trainer_state.json contient 'best_model_checkpoint' en v4)
    """
...
def extract_history(log_history: list[dict]) -> dict:
    ...
    Nouveauté v4 : "eval_bleu" est désormais toujours présent car
    compute_metrics est activé et metric_for_best_model="eval_bleu".
    """
    history = {
        "step":       [],
        "epoch":      [],
...
def plot_loss(ax, steps, train_loss, val_loss, best_step: int | None) -> None:
    ...
    ax.plot(steps, val_loss,   marker="s", linewidth=2, linestyle="--",
            color="#F07B5B", label="Validation loss")

    for x, y in zip(steps, train_loss):
        if y is not None:
            ax.annotate(f"{y:.3f}", (x, y),
                        textcoords="offset points", xytext=(0, 8),
                        ha="center", fontsize=7.5, color="#5B8AF0")
    for x, y in zip(steps, val_loss):
        if y is not None:
            ax.annotate(f"{y:.3f}", (x, y),
                        textcoords="offset points", xytext=(0, -14),
                        ha="center", fontsize=7.5, color="#F07B5B")

    # Marqueur Early Stopping
    if best_step is not None and best_step in steps:
        ax.axvline(best_step, color="#9B59B6", linestyle=":",
...
def plot_bleu(ax, steps, bleu_scores, best_step: int | None) -> None:
    ...
    st, bl = zip(*valid)
    ax.plot(st, bl, marker="D", linewidth=2, color="#52C299", label="BLEU (val)")
    ax.fill_between(st, bl, alpha=0.12, color="#52C299")
    ...
    # Marqueur Early Stopping (modèle sélectionné par le Trainer)
...
# ── Graphe 3 : Évolution epoch ─────────────────────────────────────────────────

def plot_epoch_progress(ax, steps, epochs, val_loss, bleu) -> None:
    """
    Axe double : val_loss (gauche, bleu) vs BLEU (droite, vert) par step,
    avec des bandes verticales colorées par époque.
    Utile pour visualiser à quelle époque l'Early Stopping s'est déclenché.
    """
    colors_epoch = ["#EAF4FB", "#FEF9E7", "#EAFAF1", "#FDEDEC"]

    # Bandes par époque
    epoch_vals = sorted(set(int(e) for e in epochs))
    ...
    for ep in epoch_vals:
        ep_steps = epoch_step_ranges.get(ep, [])
        if not ep_steps:
            continue
        color = colors_epoch[(ep - 1) % len(colors_epoch)]
        ax.axvspan(min(ep_steps), max(ep_steps), alpha=0.25, color=color,
                   label=f"Époque {ep}")

    # Courbe val_loss (axe gauche)
    ax.plot(steps, val_loss, marker="s", linewidth=2, linestyle="--",
            color="#F07B5B", label="Val loss")
    ax.set_xlabel("Step", fontsize=11)
    ax.set_ylabel("Validation loss", fontsize=11, color="#F07B5B")
    ax.tick_params(axis="y", labelcolor="#F07B5B")
    ...
    if valid_bleu:
        ...
        ax2.plot(sb, bb, marker="D", linewidth=2, color="#52C299", label="BLEU")
...
def plot_learning_curves(
    ...
    title: str = "Courbe d'apprentissage — ALLaM-7B (Tunisien → MSA) v4",
) -> str:
    ...
    # Mise en page : 1 ligne × 3 colonnes
    ...
    # Bandeau info paramètres v4
    param_text = (
        f"LoRA r={LORA_R}  |  dropout={LORA_DROPOUT}  |  "
...
if __name__ == "__main__":
    print(SEP)
    print("  ALLaM-7B v4 — Courbes d'apprentissage")
    print(SEP)
    ...
    if state_path is None:
        ...
        print("   Lancez d'abord finetune_allam7b_tun_msa_v4000.py.")
    else:
        ...
        if not log_history:
            ...
            if best_step:
                ...
                print("  ⚠ best_model_checkpoint absent du state "
