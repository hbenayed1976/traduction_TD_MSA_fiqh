"""
Évaluation autonome — ALLaM-7B fine-tuné (adapteur LoRA) — BEAM SEARCH (num_beams=4)
Task  : Traduction dialecte tunisien → arabe standard moderne (MSA)
Charge le modèle de base + l'adapteur LoRA sauvegardé (MEILLEURE CONFIG : r=16, 3 epochs),
puis évalue sur le test set complet (test_set_FINAL.json) en beam search,
pour une comparaison équitable avec les baselines Seq2Seq (AraT5v2, AraBART),
suite au commentaire du Reviewer 4 sur le décodage glouton (greedy) initial.

IMPORTANT : ce script ne ré-exécute QUE la meilleure configuration ALLaM-7B
(r=16, 3 epochs) — pas toutes les configs de l'ablation LoRA. Voir la
justification méthodologique fournie pour la réponse aux reviewers.
...
# ⚠️ À ADAPTER : pointer vers le checkpoint de la MEILLEURE CONFIG (r=16, 3 epochs),
# pas vers le dossier de sortie générique du script d'entraînement (qui utilisait r=64, 4 epochs).
LORA_DIR     = r"C:\Users\infocom\tradiction_dialect\allam7b_tun_msa_output-3000_r16_e3"

TEST_FILE    = os.path.join(LORA_DIR, "test_set_FINAL.json")
OUTPUT_DIR   = LORA_DIR          # résultats écrits au même endroit
SEED         = 42

# ── Paramètres de décodage — beam search (Reviewer 4) ─────────
NUM_BEAMS         = 4
DECODING_STRATEGY = f"beam_search_num_beams={NUM_BEAMS}"
...
def translate_to_msa(item: dict, max_new_tokens: int = 128) -> str:
    ...
    with torch.no_grad():
        ...
        outputs = model.generate(
            ...
            num_beams=NUM_BEAMS,        # ← beam search (au lieu de greedy)
            do_sample=False,            # obligatoire : pas de sampling en beam search pur
            early_stopping=True,
...
summary_metrics = {
    ...
    "lora_config"       : "r=16, 3 epochs (meilleure config)",
...
# Suffixe _beam4 pour ne PAS écraser les résultats greedy déjà produits
results_path = os.path.join(OUTPUT_DIR, "evaluation_results_beam4.json")
...
# ── Appel export ──────────────────────────────────────────────
excel_path = os.path.join(OUTPUT_DIR, "allam_evaluation_expert_beam4.xlsx")
export_excel(results_eval, summary_metrics, excel_path)

print(f"\n💾 JSON  → {results_path}")
print(f"📊 Excel → {excel_path}")
print(f"📄 Test  → {TEST_FILE}")
print(f"\n🔎 Stratégie de décodage : {DECODING_STRATEGY}")
print(f"🔎 Config LoRA           : r=16, 3 epochs (meilleure config)")
print("\n" + SEP)
print("  ✅ ÉVALUATION ALLaM-7B (BEAM SEARCH) TERMINÉE AVEC SUCCÈS!")
print(SEP)

# ══════════════════════════════════════════════════════════════
# 8. COMPARATIF RAPIDE avec les résultats greedy (si disponibles)
# ══════════════════════════════════════════════════════════════
greedy_path = os.path.join(OUTPUT_DIR, "evaluation_results.json")
if os.path.exists(greedy_path):
    with open(greedy_path, "r", encoding="utf-8") as f:
        greedy_summary = json.load(f)

    print("\n" + SEP)
    print("  📊 COMPARATIF GREEDY vs BEAM SEARCH (num_beams=4)")
    print(SEP)
    metrics_to_compare = [
        ("BLEU (corpus)",   "corpus_bleu"),
        ("chrF++ (corpus)", "corpus_chrf_pp"),
        ("TER (corpus, ↓)", "corpus_ter"),
        ("Exact Match Acc", "exact_match_acc"),
    ]
    for label, key in metrics_to_compare:
        g_val = greedy_summary.get(key, float("nan"))
        b_val = summary_metrics.get(key, float("nan"))
        delta = b_val - g_val
        print(f"  {label:<18s}  greedy={g_val:>8.4f}   beam4={b_val:>8.4f}   "
              f"Δ={delta:+.4f}")
    print(SEP)
else:
    print(f"\n⚠️  Résultats greedy introuvables ({greedy_path}) — "
          "lancez d'abord l'évaluation greedy pour obtenir le comparatif.")
